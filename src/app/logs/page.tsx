...
export default function Logs() {
  ...
  const [, setTemPermissao] = useState<boolean | null>(null);
  ...
  const carregarLogs = async (pagina: number, empresaId: string) => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_URL_API}/logs?page=${pagina}&limit=${logsPorPagina}&empresaId=${empresaId}`);
  ...
  useEffect(() => {
    ...
    const initialize = async () => {
      ...
      try {
        ...
        const logsData = await carregarLogs(1, usuarioData.empresaId);
  ...
  const extrairInfoVenda = (descricao: string) => {
    ...
    const quantidadeMatch = descricao.match(/|\sQuantidade:\s(\d+)/i) || descricao.match(/|\sQty:\s(\d+)/i);
    const clienteMatch = descricao.match(/|\sCliente:\s(.+?)(\n|\$)/i) || descricao.match(/|\sClient:\s(.+?)(\n|\$)/i);
  ...
  const mudarPagina = async (novaPagina: number) => {
    ...
    try {
      ...
      if (usuarioData && usuarioData.empresaId) {
        const logsData = await carregarLogs(novaPagina, usuarioData.empresaId);
        await carregarNomesUsuarios(logsData);
      }
    } catch (error) {
  ...
  const logsFiltrados = logs.filter((log) => log.descricao.toLowerCase().includes(busca.toLowerCase()));
      ...
      <div className="w-full max-w-6xl">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center gap-2 md:gap-4 mb-3 md:mb-6">
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm font-mono" style={{ color: temaAtual.texto }}>
              {totalLogs} Logs
            </div>
          </div>

          {totalLogs > logsPorPagina && (
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <button onClick={() => mudarPagina(paginaAtual - 1)} disabled={paginaAtual === 1} className={`p-2 rounded-full transition-all ${paginaAtual === 1 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-200 dark:hover:bg-gray-700"}`} style={{ color: temaAtual.texto }}>
                  <FaAngleLeft />
                </button>

                <span className="text-sm font-mono min-w-[60px] text-center" style={{ color: temaAtual.texto }}>
                  {paginaAtual} / {totalPaginas}
                </span>

                <button onClick={() => mudarPagina(paginaAtual + 1)} disabled={paginaAtual === totalPaginas || totalPaginas === 0} className={`p-2 rounded-full transition-all ${paginaAtual === totalPaginas || totalPaginas === 0 ? "opacity-50 cursor-not-allowed" : "hover:bg-gray-200 dark:hover:bg-gray-700"}`} style={{ color: temaAtual.texto }}>
                  <FaAngleRight />
                </button>
              </div>
            </div>
