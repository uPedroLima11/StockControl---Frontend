...
export default function PedidosPage() {
  ...
  const filtrarPedidos = useCallback(() => {
    ...
    if (busca) {
      ...
      filtrados = filtrados.filter((pedido) => pedido.numero.toLowerCase().includes(termo) || pedido.fornecedor.nome.toLowerCase().includes(termo) || pedido.itens.some((item) => item.produto.nome.toLowerCase().includes(termo)));
    }
  ...
  if (carregando) {
    return (
      <div className="flex items-center justify-center min-h-screen" style={{ backgroundColor: temaAtual.fundo }}>
        <div className="text-lg" style={{ color: temaAtual.texto }}>
          {t("carregando")}
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen p-4 md:p-8" style={{ backgroundColor: temaAtual.fundo }}>
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex-1 flex items-center border rounded-full px-3 py-1 h-10 mr-2" style={{ borderColor: temaAtual.borda, maxWidth: 320 }}>
            <input type="text" placeholder={t("buscarPedidos")} className="outline-none bg-transparent placeholder-gray-400 flex-1 text-sm" style={{ color: temaAtual.texto }} value={busca} onChange={(e) => setBusca(e.target.value)} />
          ...
          {totalPaginas > 1 && (
            <div className="flex items-center ml-0 md:ml-2">
              <div className="flex items-center gap-2 bg-gray-800/60 backdrop-blur-md rounded-lg p-1 border border-gray-700/30">
                <button onClick={() => mudarPagina(paginaAtual - 1)} disabled={paginaAtual === 1} className={`p-1 rounded-full cursor-pointer ${paginaAtual === 1 ? "opacity-50 cursor-not-allowed" : "hover:opacity-80"}`} style={{ color: temaAtual.texto }}>
                  <FaChevronLeft />
                </button>
                ...
                <button onClick={() => mudarPagina(paginaAtual + 1)} disabled={paginaAtual === totalPaginas} className={`p-1 rounded-full cursor-pointer ${paginaAtual === totalPaginas ? "opacity-50 cursor-not-allowed" : "hover:opacity-80"}`} style={{ color: temaAtual.texto }}>
                  <FaChevronRight />
                </button>
              </div>
            </div>
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className={`px-2 py-1 rounded text-xs ${statusInfo.cor}`}>{statusInfo.texto}</span>
                        <span
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <button
                        ...
                        className={`px-3 py-1 rounded flex items-center gap-1 text-sm ${pedido.status === "CONCLUIDO" || pedido.status === "CANCELADO" || enviandoEmail[pedido.id] || !podeEnviarEmail ? "" : "cursor-pointer"}`}
                        style={{
                        ...
                        title={!podeEnviarEmail ? t("semPermissaoEnviarEmail") || "Você não tem permissão para Enviar Email" : pedido.status === "CONCLUIDO" || pedido.status === "CANCELADO" ? t("naoPodeEnviarEmailStatus") || "Não é possível enviar email para pedidos concluídos ou cancelados" : undefined}
                      >
                        {enviandoEmail[pedido.id] ? <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white"></div> : <FaEnvelope />}
                      ...
                      {podeGerenciarStatus && pedido.status !== "CONCLUIDO" && pedido.status !== "CANCELADO" && (
                        <div className="flex gap-1 flex-wrap">
                          {["PENDENTE", "PROCESSANDO", "CONCLUIDO", "CANCELADO"].map((status) => (
                            <button key={status} onClick={() => handleAtualizarStatus(pedido.id, status)} className={`px-2 cursor-pointer py-1 rounded text-xs ${pedido.status === status ? (status === "CANCELADO" ? "bg-red-500 text-white" : "bg-gray-500 text-white") : status === "CANCELADO" ? "bg-red-200 text-red-700 hover:bg-red-300 dark:bg-red-900 dark:text-red-300 dark:hover:bg-red-800" : "bg-gray-200 text-gray-700 hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-300 dark:hover:bg-gray-600"}`}>
      ...
      {modalAberto && modalTipo === "criar" && <CriarModal temaAtual={temaAtual} t={t} fornecedores={fornecedores} produtos={produtos} fornecedorSelecionado={fornecedorSelecionado} itensCriacao={itensCriacao} observacoesCriacao={observacoesCriacao} buscaProduto={buscaProduto} carregandoCriacao={carregandoCriacao} setFornecedorSelecionado={setFornecedorSelecionado} setBuscaProduto={setBuscaProduto} setObservacoesCriacao={setObservacoesCriacao} adicionarItem={adicionarItem} removerItem={removerItem} atualizarQuantidade={atualizarQuantidade} atualizarObservacao={atualizarObservacao} calcularTotal={calcularTotal} handleCriarPedido={handleCriarPedido} onClose={handleFecharModal} />}
      ...
      {modalAberto && modalTipo === "detalhes" && pedidoSelecionado && (
        <div className="fixed inset-0 flex items-center justify-center z-50" style={{ background: "rgba(0, 0, 0, 0.75)" }}>
          ...
          >
            <div className="flex justify-between items-start mb-4">
              <h2 className="text-xl font-bold">
                {t("detalhesPedido")} - {pedidoSelecionado.numero}
              </h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    <strong>{t("fornecedor")}:</strong> {pedidoSelecionado.fornecedor.nome}
                  </p>
                  <p>
                    <strong>{t("emailFornecedor")}:</strong> {pedidoSelecionado.fornecedor.email}
                  </p>
                  <p>
                    <strong>{t("status.titulo") || "Status"}:</strong> {t(`status.${pedidoSelecionado.status.toLowerCase()}`)}
                  </p>
                  <p>
                    <strong>{t("dataSolicitacao")}:</strong> {formatarData(pedidoSelecionado.dataSolicitacao)}
                  </p>
                  <p>
                    <strong>{t("ultimaAtualizacao")}:</strong> {formatarData(pedidoSelecionado.dataAtualizacao)}
                  </p>
                  <p>
                    <strong>{t("solicitante")}:</strong> {pedidoSelecionado.usuario.nome}
                  </p>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-1 text-sm">
                  <p>
                    <strong>{t("totalItens")}:</strong> {pedidoSelecionado.itens.length}
                  </p>
                  <p>
                    <strong>{t("totalSolicitado")}:</strong> {pedidoSelecionado.itens.reduce((sum, item) => sum + item.quantidadeSolicitada, 0)}
                  </p>
                  <p>
                    <strong>{t("totalAtendido")}:</strong> {pedidoSelecionado.itens.reduce((sum, item) => sum + item.quantidadeAtendida, 0)}
                  </p>
                  <p>
                    <strong>{t("valorTotal")}:</strong> R$ {pedidoSelecionado.total.toFixed(2)}
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <div key={item.id} className="p-3 border rounded" style={{ borderColor: temaAtual.borda }}>
                    <div className="flex justify-between items-start mb-2">
                      <div className="flex-1">
                        ...
                        <p className="text-sm">
                          R$ {item.precoUnitario.toFixed(2)} {t("unidade")}
                        </p>
                      </div>
            ...
            <div className="flex flex-wrap gap-3">
              ...
              {podeEditarPedido && pedidoSelecionado.status !== "CONCLUIDO" && pedidoSelecionado.status !== "CANCELADO" && (
                <div className="flex flex-wrap gap-2">
                  <button onClick={() => handleAtualizarStatus(pedidoSelecionado.id, "CANCELADO")} className="px-4 py-2 cursor-pointer rounded text-white bg-red-500 hover:bg-red-600">
                    <FaTimes className="inline mr-2" />
                    {t("cancelarPedido")}
                  </button>
                  ...
                  <button onClick={() => handleConcluirPedidoComEstoque(pedidoSelecionado.id)} className="px-4 py-2 rounded cursor-pointer text-white bg-green-600 hover:bg-green-700">
